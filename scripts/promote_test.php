...
function promote($site_id) {
	...
	echo "links_queue: $links_queue, links_queue_all: $links_queue_all\n";
	...
	echo "hours: $hours, from_time: $from_time\n";
	echo "last_published: $last_published, links_published: $links_published, links_published_projection: $links_published_projection\n";
	...
	echo "past_karma_long: $past_karma_long, past_karma_short: $past_karma_short, past_karma: $past_karma, min_past_karma: $min_past_karma, last_resort_karma: $last_resort_karma\n";
	...
	echo "users_karma_avg: ".$globals['users_karma_avg']."\n";
	...
	echo "SELECT SQL_NO_CACHE link_id, link_karma as karma from sub_statuses, links, users  where $where $sort LIMIT 30\n";
	...
	if ($links) {
		...
		foreach($links as $dblink) {
			...
			echo "$link->votes GT= $min_votes AND $link->karma GT= $karma_threshold AND $published < $max_to_publish\n";
...
function publish($site, $link) {
	...
	//$db->commit();
	echo "update links set link_status='published', link_date=now(), link_votes_avg=$link->votes_avg where link_id=$link->id\n";
	...
	if (! $site_info->sub ) {   //|| $site_info->visible ) { // Only post if it's not a sub or it's visible (dmnm in mnm, f.e.)
		...
		echo "calling: ".dirname(__FILE__)."/post_link.php $site_info->name $link->id published";
